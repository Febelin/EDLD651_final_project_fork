...
# Fruit cleaning 
# Febe: To make the code cleaner and more readable, I would like to suggest combining the multiple filter() statements into a single pipeline using |> and a regular expression.
...
```{r}
# Febe: I really admire the authors' willingness to tackle such a messy dataset. It requires a lot of extra work to clean, and this code chunk clearly reflects the effort and care put into the process. Great work!
...
# Febe: As a small suggestion, since the fruit and vegetable pipelines are almost identical, you might consider wrapping the “family + summary + ANOVA” workflow into a helper function and reusing it for both datasets. This could reduce duplication and make the script even easier to maintain.
...
```
```{r}
#Febe: Perhaps as an alternative, we could also consider plotting the mean price by form instead of filling by `name`, since the many colors might make the current plot a bit harder to interpret. 
